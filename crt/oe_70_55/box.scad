...
screw_width_bottom = plate_width - 16;
h = 90;
z = 2.5 + h / 2;
pcb_x = 35;
...
module tube_holder_all() {
	difference() {
		...
		for (i=[-1,1]) {
			...
			translate([0, screw_width_bottom / 2 * i, 0])
				cylinder(h=50, d=5.25, center=true);
...
module plate() {
	...
	difference() {
		...
		for (i=[-1,1]) {
			...
			// translate([-plate_l / 2 + plate_x + 14, screw_width_bottom / 2 * i, 0])
			// 	cylinder(h=10, d=5.25, center=true);
			// translate([-plate_l / 2 + plate_x + 62, screw_width_bottom / 2 * i, 0])
			// 	cylinder(h=10, d=5.25, center=true);

			// UI board holes
			translate([plate_l / 2 - 11 + plate_x, screw_width_bottom / 2 * i, 0])
				cylinder(h=10, d=5.25, center=true);
